...
class Model(object):
  ...
  def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
    ...
    hidden_units = 300
    user_emb = np.zeros((user_count,hidden_units))
    item_emb = np.zeros((item_count,hidden_units))
    cate_emb = np.zeros((cate_count,hidden_units))
    with open("glove/hard_item_emb.txt","r") as f:
      lines=f.readlines()
      i=0
      for line in lines:
        emb = line.split()[1:]
        for j in range(300):
          item_emb[i][j]=float(emb[j])
        i+=1
    
    with open("glove/hard_category_emb.txt","r") as f:
      lines=f.readlines()
      i=0
      for line in lines:
        emb = line.split()[1:]
        for j in range(300):
          cate_emb[i][j]=float(emb[j])
        i+=1
    
    with open("glove/hard_user_emb.txt","r") as f:
      lines=f.readlines()
      i=0
      for line in lines:
        emb = line.split()[1:]
        for j in range(300):
          user_emb[i][j]=float(emb[j])
        i+=1
    print("user_emb[0][0]:",user_emb[0][0])
    user_emb_w=tf.convert_to_tensor(user_emb,dtype=tf.float32,name="user_emb_w")
    # user_emb_w = tf.get_variable("user_emb_w", [user_count, hidden_units])
    item_emb_w=tf.convert_to_tensor(item_emb,dtype=tf.float32,name="item_emb_w")
    # item_emb_w = tf.get_variable("item_emb_w", [item_count, hidden_units])
    
    item_b = tf.get_variable("item_b", [item_count],
                             initializer=tf.constant_initializer(0.0))
    cate_emb_w=tf.convert_to_tensor(cate_emb,dtype=tf.float32,name="cate_emb_w")
    ...
    # print("after concat i_emb:",i_emb.shape)
    i_emb = tf.layers.dense(i_emb,hidden_units)
    ...
    # print("after concat j_emb:",j_emb.shape)
    j_emb = tf.layers.dense(j_emb,hidden_units)
    ...
    # print("after concat h_emb:",h_emb.shape)
    h_emb = tf.layers.dense(h_emb,hidden_units) 
    ...
    # print(item_emb_all.shape)!!!!!!!!!!!!!!
    item_emb_all=tf.layers.dense(item_emb_all,300)
...
def attention(queries, keys, keys_length):
  '''
    ...
    keys_length: [B]
  '''
  # print("start attention option...")
  # print("queris shape",queries.shape)
  # print("keys shape:",keys.shape)
  # queries=tf.layers.dense(queries,300)
  # keys = tf.layers.dense(keys,300)
  # queries=tf.layers.dense(queries.shape,queries.shape/2)
  # keys = tf.layers.dense(keys.shape,keys.shape/2)
...
def attention_multi_items(queries, keys, keys_length):
  ...
  # print(1111)
  # outputs = tf.layers.dense(outputs,queries_hidden_units/2)
  print(outputs.get_shape().as_list())
  return outputs
